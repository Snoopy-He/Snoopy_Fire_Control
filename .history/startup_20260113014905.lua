...
function preciseTimer(time)
    os.sleep(time)
end

function main_loop(parameter)
    Target_Update(parameter)
    Motor_Calc(parameter)
    print("tp: "..parameters.Gimbal.Pitch_TarAng.."cp:"..Parameters.Gimbal.Pitch_CurAng)
end

Init(Parameters,"AutoCannon","Player","front","back")
Cannon_Position_Update(Parameters)
while true do
    parallel.waitForAll(Target_Update(Parameters)
