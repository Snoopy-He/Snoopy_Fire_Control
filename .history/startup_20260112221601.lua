...
function Init(parameter,cannontype,enable)
...
function Predictor_Calc(targets, flying_time)
...
function Target_Update(cannon_pos,player_targets,monster_targets,dist_min,dist_max)
    ...
    for k, v in pairs(monsters) do
        monster_targets[i] = Monster_Target_Update(v,cannon_pos,dist_min,dist_max,monster_targets)
        i = i+1
    end
...
function Track_Calc(parameter)   --弹道计算
    ...
    parameter.Location.Target.Distance_3D = Distance_3D_Calc(parameter.Location.Cannon.X, parameter.Location.Cannon.Y, parameter.Location.Cannon.Z, parameter.Location.Target.X, parameter.Location.Target.Y, parameter.Location.Target.Z),
    parameter.Location.Target.Distance_2D = w
