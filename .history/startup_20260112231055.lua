...
function Fx(p1,p2,p3,p4,p5,a,x) 
    return p1*math.sec(x)+p2*math.tan(x)+a*math.log(p3-p4*sec(x))+p5 
end

function fx_derivative(p1,p2,p3,x)
    return math.sec(x)*(p1*math.tan(x)*(1+1/(p1*math.sec(x)-p3))+p2*sec(x))
end

function Fx_derivative(p1,p2,p3,p4,a,x)
    return math.sec(x)*(math.tan(x)*(p1-a*p4/(p3-p4*math.sec(x)))+p2*sec(x))
end
