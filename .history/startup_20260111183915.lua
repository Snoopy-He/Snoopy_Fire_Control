...
function Predict_Calc(target_pos, flying_time)
    ...
    return {
        X = target_pos.X + (target_vx + target_vpx)/2 * flying_time * 1.4,
        Y = target_pos.Y + (target_vy + target_vpy)/2 * flying_time * 1.4,
        Z = target_pos.Z + (target_vz + target_vpz)/2 * flying_time * 1.4
    }
end
