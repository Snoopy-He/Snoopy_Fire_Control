...
function Vertical_Height_Calc(pitch, time)
    ...
    if math.abs(1 - drag) < 0.001 then
        local height = velocity * time - 0.5 * gravity * time * time
        -- 如果您需要返回相对高度（从炮口开始）：
    ...
    local height = height_int + t_frac * (height_next - height_int) - y0
    return height
end
--[[
    local index = 1
...
function Binary_Search_Pitch(target_height, distance)
    ...
    while left <= right do
        ...
        if math.abs(height - target_height) < 0.014 then
            best_pitch = pitch
            print("height: "..height.."target_height: "..target_height)
            break
