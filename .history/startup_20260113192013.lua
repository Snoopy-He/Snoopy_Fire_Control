...
function Fx_derivative(p1,p2,p3,p4,p5,p6,x)
    return math.sec(x)*(math.tan(x)*p1+math.sec(x)*p2-p3*e*math.tan(x)/(p4-e*math.sec(x)))
end
...
function Newton_Raphson(p1,p2,p3,p4,p5,p6,x0,n)
    ...
    for i=1,n do
        ...
        local f_derivative = Fx_derivative(p1,p2,p3,p4,p5,p6,x)
...
function Track_Calc(parameter)   --弹道计算
    local n,m,v,d,T,g,l = 6,40,9,0.01,0.05,0.05,6
    ...
    --local a3 = m*n*T+d*l
    --local a4 = d*w
    --local a5 = -k*l/d/n-a*math.log(m*n*T)-h
    local a1 = g*w/d/v
    local a2 = w
    local a3 = g/d/d
    local a4 = 1+d*l/v
    ...
    print("h:"..parameter.Location.Target.Y)
