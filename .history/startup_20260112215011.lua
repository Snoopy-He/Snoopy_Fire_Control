...
function Cannon_Position_Update(parameter)
    ...
    parameter.Location.Cannon.X = selfPos.x + para.Location.Cannon_Offset.X,
    parameter.Location.Cannon.Y = selfPos.y + para.Location.Cannon_Offset.Y,
    parameter.Location.Cannon.Z = selfPos.z + para.Location.Cannon_Offset.Z
end
...
function Track_Calc(parameter)   --弹道计算
    ...
    local w = Distance_2D_Calc(cannon.X,cannon.Z, target.X,target.Z)
    local h = target.Y-cannon.Y
    local a = k*m*T/d/d
    local a1=k*w/d/n
    local a2=w
    local a3=m*n*T+d*l
