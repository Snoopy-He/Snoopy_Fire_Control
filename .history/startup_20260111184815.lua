...
function Flying_Time_Calc(pitch,distance)
    ...
    local real_distance = distance -cannon_length*math.cos(math.rad(pitch))
    local velocity = 180/20
    local drag = 0.01
    local gravity = 0.05
    local result
    if drag < 0.001 then
        result = real_distance / (velocity * math.cos(math.rad(pitch)))
    else
...
function Binary_Search_Pitch(target_height, distance)
    ...
    while left <= right do
        ...
        local time = Flying_Time_Calc(math.deg(pitch), distance)
        local height = Vertical_Height_Calc(math.deg(pitch), time)
        if math.abs(height - target_height) < 0.014 then
            best_pitch = pitch
            print("Found pitch: "..math.deg(best_pitch).." for target height: "..target_height.." at distance: "..distance)
            break
