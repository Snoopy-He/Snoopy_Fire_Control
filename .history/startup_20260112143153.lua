...
function Player_Target_Update(target,cannon,dist_min,dist_max)
    return {
        x = target.x,
        y = target.y,
        z = target.z,
...
function Monster_Targets_Fliter(monsters,count)
    local length = #monsters
    local swapped = false
    for i = 1, length do
        swapped = false
        for j = 1, length - i do
            if monsters[j].Distance > monsters[j+1].Distance then
                monsters[j], monsters[j + 1] = monsters[j + 1], arr[j]
                swapped = true
            end
        end
...
function Player_Targets_Fliter(players,count)
    local length = #players
end

local Player_Targets = {}
local Monster_Targets = {}
function Target_Update(cannon_pos,dist_min,dist_max)
    ...
    for k, v in pairs(players) do
        ...
        Player_Targets[i] = Player_Target_Update(v,cannon_pos,dist_min,dist_max)
        i=i+1
    end
    i = 1
