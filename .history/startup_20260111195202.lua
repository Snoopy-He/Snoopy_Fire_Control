...
function Vertical_Height_Calc(pitch, time)
    ...
    if math.abs(drag) < 0.001 then
        return velocity * time - 0.5 * gravity * time * time - y0
    end
--[[
    local one_minus_drag = 1 - drag                 -- 0.01
    local drag_pow_time = math.pow(drag, time)
    local one_minus_drag_pow_time = 1 - drag_pow_time

    local term1 = -gravity * time / one_minus_drag
    local term2 = (velocity + gravity / one_minus_drag) * one_minus_drag_pow_time / one_minus_drag
