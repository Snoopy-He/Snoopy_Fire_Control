...
function Player_Target_Update(target,cannon,dist_min,dist_max)
    return {
        ...
        y = target.y,
        z = target.z,
        distance = clamp(Distance_3D_Calc(target.x,target.y,target.z,cannon.X,cannon.Y,cannon.Z),dis),
...
function Monster_Target_Update(target,cannon)
    return {
        x = target.x,
        y = target.y,
        z = target.z,
        distance = Distance_3D_Calc(target.x,target.y,target.z,cannon.X,cannon.Y,cannon.Z),
    }
end
...
function Target_Update(cannon_pos)
    ...
    for k, v in pairs(players) do
        ...
        Player_Targets[i] = Player_Target_Update(v,cannon_pos)
        i=i+1
    end
